...
SELECT SUM(QUANTIDADE), D.NOME_DISCIPLINA FROM FATO_FREQUENCIA FQ JOIN DIM_TURMA T
		ON(FQ.ID_ESCOLA = T.COD_TURMA) JOIN
		DIM_STATUS_FREQUENCIA ST
		ON(ST.COD_FREQUENCIA = FQ.ID_STATUS_FREQUENCIA) JOIN
		DIM_DISCIPLINA D ON(D.COD_DISCIPLINA = FQ.ID_DISCIPLINA)

		WHERE ST.STATUS_FREQUENCIA = 'F'
